...
#define MAX_ACCEL 1 //desired maximum possible acceleration (absolute value)
#define ACCEL_FACTOR 0.25 //increases acceleration when increaesd (no units)
...
float changeVelocity(float vCurrent, float vBegin, float vEnd, float *tOld)
{
  float vNext;
  ...
  if(vEnd > vBegin) {
    if(vCurrent < vEnd) {
      vNext = vCurrent + MAX_ACCEL * (PI/5) * atan(ACCEL_FACTOR * (vEnd - vBegin)); 
    } else {
      vNext = vEnd;
    }
  } else if(vEnd < vBegin) {
...
void loop()
{
  ...
  if(Serial.available() > 0) {
     ...
     velocityIn = Serial.parseFloat();
  }
  
  //linear velocity change to input velocity value
  /*if(velocityIn != velocityTarget) {
    velocityTarget = changeVelocity(velocityTarget, velocityOld, velocityIn, &tOldV);
  }*/
